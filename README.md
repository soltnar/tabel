...
- Windows 10/11: скачать автономный архив релиза
  `tabel_windows_portable_vX.Y.Z.zip`, полностью распаковать его и дважды
  нажать `Tabel.exe`. Установка Python и библиотек не требуется.

Старый кроссплатформенный архив `tabel_local_vX.Y.Z.zip` по-прежнему содержит
`START_WINDOWS.bat`. На первом запуске ему нужен интернет: запускатель использует
`winget`, а при
...
Для компьютеров, где установка Python запрещена, используйте автономную
Windows-сборку с `Tabel.exe`. Подробности находятся в
`ЛОКАЛЬНЫЙ_ЗАПУСК_WINDOWS.txt`.
